...
const Highlightes = ({ assignment }) => {
    const { _id, title, thumbnail, description, marks, date } = assignment;
    return (
        <div className="border border-gray-300 bg-indigo-50 dark:border-gray-700 rounded hover:-translate-y-2 shadow duration-1000 ">
            ...
            <div className="p-6">
                ...
                <Link to={`/assignmentDetails/${_id}`} className="bg-indigo-500 border-2 border-indigo-500 px-[16px] py-[6px] text-gray-100 duration-500 font-medium tracking-wide rounded hover:bg-transparent hover:text-indigo-600">View More <FaArrowRightLong className="inline"/></Link>
            </div>
        </div>
    )
}
